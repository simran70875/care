...
const carerPositions = [
  "All Positions", "Care Worker", "Senior Care Worker", "Coordinator", "Admin", "HR"
];
const areas = ["Select Area", "Area A", "Area B", "Area C"];

// Sample data exactly matching the visible columns in the screenshot
const dummyRows: CarerAvailabilityEntry[] = [
  { id: 1, carerName: "Ahmad Fraz", position: "Care Worker", scheduleConflicts: "No Conflict", daysAvailable: 100, daysAvailableFromSchedule: 100, daysSelected: "—" },
  { id: 2, carerName: "Amina Abdalla", position: "Care Worker", scheduleConflicts: "No Conflict", daysAvailable: 100, daysAvailableFromSchedule: 100, daysSelected: "—" },
  { id: 3, carerName: "Arfia Qaisar", position: "Care Worker", scheduleConflicts: "No Conflict", daysAvailable: 100, daysAvailableFromSchedule: 100, daysSelected: "—" },
  { id: 4, carerName: "Ashdeep Singh", position: "Care Worker", scheduleConflicts: "No Conflict", daysAvailable: 100, daysAvailableFromSchedule: 100, daysSelected: "—" },
  { id: 5, carerName: "Asma Mobeen Ahmed", position: "Care Worker", scheduleConflicts: "No Conflict", daysAvailable: 100, daysAvailableFromSchedule: 100, daysSelected: "—" },
  { id: 6, carerName: "Bhushan Mehra", position: "Care Worker", scheduleConflicts: "No Conflict", daysAvailable: 0, daysAvailableFromSchedule: 100, daysSelected: "—" },
  { id: 7, carerName: "Bunni Christiana Junaid", position: "Care Worker", scheduleConflicts: "No Conflict", daysAvailable: 100, daysAvailableFromSchedule: 100, daysSelected: "—" },
  { id: 20, carerName: "Dilsha Bibi", position: "Care Worker", scheduleConflicts: "Conflict", daysAvailable: 0, daysAvailableFromSchedule: 0, daysSelected: "—" },
  { id: 21, carerName: "Deepali Singh", position: "Senior Care Worker", scheduleConflicts: "Conflict", daysAvailable: 0, daysAvailableFromSchedule: 0, daysSelected: "—" },
];


// Helper function for rendering the SHORT progress bars (matches screenshot visual)
const renderShortProgressBar = (value: number, color: string, label: string) => {
    ...
    return (
        <Box className="flex flex-col items-center justify-center">
            ...
            >
                ...
                <Typography variant="caption" className="font-semibold" sx={{ color: DARK_GREEN, fontSize: '10px' }}>
                    {label}
                </Typography>
            </Box>
            {/* The 100% text directly below the bar */}
            <Typography variant="caption" className="font-semibold" sx={{ color: DARK_GREEN, fontSize: '10px', marginTop: '2px' }}>
                {`${value}%`}
            </Typography>
        </Box>
    );
...
const columns: GridColDef<CarerAvailabilityEntry>[] = [
  { 
    ...
    width: 200, 
    renderCell: (params) => (
      <Typography variant="body2" className="text-blue-700 font-medium cursor-pointer">
        {params.value}
      </Typography>
    ),
  },
  { 
    ...
    renderHeader: () => (
        <Box className="flex items-center">
            <Typography variant="body2" className="font-bold mr-1">Position</Typography>
            <Typography variant="caption" sx={{ color: ACTION_CYAN }}>▲</Typography> {/* Cyan/Teal arrow */}
        </Box>
    ),
  },
  { 
  ...
  { 
    ...
    renderCell: (params) => renderShortProgressBar(params.value, ACCENT_GREEN, 'Mon'), 
  },
  { 
    field: "daysAvailable", 
    ...
    renderCell: (params) => {
        ...
        return <Typography variant="body2">{params.value === 0 ? "—" : `${params.value}%`}</Typography>;
    },
  },
  { field: "daysSelected", headerName: "Selected", width: 120, renderCell: (params) => (
...
const CarerAvailability: React.FC = () => {
  ...
  const [date, setDate] = useState("01-12-2025"); // Using DD-MM-YYYY format from screenshot
    ...
    <Box className="bg-white min-h-screen">
      ...
      <Box className="p-4 flex flex-col gap-4 border-b border-gray-200">
        ...
        <div className="flex items-center gap-4">
            ...
            />
            <Typography variant="body2" className="text-gray-600 font-semibold">to</Typography>
            <TextField
                size="small"
                type="time"
                ...
                {daysOfWeek.map(day => (
                    <Button
                        ...
                        sx={{ 
                            ...
                            backgroundColor: selectedDays.includes(day) ? UI_PURPLE_BG : 'white',
                            border: 'none',
                            '&:hover': {
                                backgroundColor: selectedDays.includes(day) ? UI_PURPLE_BG : LIGHT_GRAY,
                                border: 'none',
                            },
            ...
            <div className="relative">
                ...
                >
                    <span className="truncate text-sm">
                        {selectedPositions.length === 0 ? "Select Position" : `${selectedPositions.length} Selected`}
                    </span>
                </Button>
                                ...
                                <FormControlLabel
                                    ...
                                    control={
                                        <Checkbox
                                            ...
                                            size="small"
                                            sx={{ color: ACTION_CYAN, '&.Mui-checked': { color: ACTION_CYAN } }}
                                            className="p-1"
                                        />
                                    }
                                    label={<Typography variant="body2" className="text-gray-700 text-sm">{position}</Typography>}
      ...
      {/* --- Action/Toolbar Bar --- */}
      <Box className={`p-3 flex justify-between items-center border-b border-gray-200 ${LIGHT_GRAY}`}>
            ...
            {toolbarButtons.map((label) => (
                <Button
                    ...
                    sx={{
                        minWidth: 'auto',
                        padding: '4px 12px',
                        borderColor: label === "Export Excel" ? UI_PURPLE_TEXT : '#D1D5DB', 
