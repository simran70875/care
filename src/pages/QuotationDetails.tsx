...
export default function QuotationDetailsPage() {
  ...
  const calculateSubtotal = () =>
    editableProducts?.reduce((sum, p) => sum + (p.totalPrice ?? 0), 0);
  ...
  const handleSendQuotation = async () => {
    try {
      ...
      if (hasInvalidPrice) {
        return setShowError(
          "Cannot send quotation: one or more products are missing prices."
        );
      }

      if (!order.subtotal || order.subtotal <= 0) {
        return setShowError(
          "Failed to send quotation: subtotal must be greater than 0."
        );
      }
  ...
  return (
    <div className="space-y-4">
      {showSuccess != "" && (
        <Alert variant="success" title={showSuccess} message={showSuccess} />
      )}
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow p-6 mb-8">
        ...
        <table className="w-full text-sm text-left border border-gray-200 dark:border-gray-700">
          <tbody>
            ...
            <tr className="border-b dark:border-gray-700">
              <th className="p-2 font-medium bg-gray-50 dark:bg-gray-800">
                Email
              </th>
          ...
          <table className="w-full text-sm text-left border border-gray-200 dark:border-gray-700">
            <tbody>
              ...
              <tr className="border-b dark:border-gray-700">
                ...
                <td className="p-2">{data?.agent?.email}</td>
              </tr>
              <tr className="border-b dark:border-gray-700">
                <th className="p-2 font-medium bg-gray-50 dark:bg-gray-800">
                  Phone
                </th>
                <td className="p-2">{data?.agent?.phone}</td>
              </tr>
              <tr>
                <th className="p-2 font-medium bg-gray-50 dark:bg-gray-800">
                  Address
                </th>
                <td className="p-2">{data?.agent?.address}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <tbody>
            {[
              ...
              <tr key={i} className="border-b dark:border-gray-700">
                <th className="p-2 font-medium bg-gray-50 dark:bg-gray-800 w-48">
                  {label}
                </th>
        ...
        <div className="grid grid-cols-3 gap-4 text-sm text-gray-800 dark:text-gray-200">
          <div className="bg-gray-50 dark:bg-gray-800 rounded p-3 shadow-sm">
            ...
            <p className="text-green-600 font-semibold">
              ...
              {editableProducts
                ?.reduce(
                  (acc, p) => acc + (p.grossProfit ?? 0) * (p.quantity ?? 0),
                  0
                )
                .toFixed(2)}
            </p>
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded p-3 shadow-sm">
            ...
            <p className="text-blue-600 font-semibold">
              ...
              {editableProducts
                ?.reduce(
                  (acc, p) => acc + (p.commission ?? 0) * (p.quantity ?? 0),
                  0
                )
                .toFixed(2)}
            </p>
      ...
      {data?.status == "Pending" && (
        <div className="flex justify-start gap-3 mt-4">
          ...
          <Button variant="primary" size="sm" onClick={handleSendQuotation}>
            Send Quotation
          </Button>
        </div>
      )}
      {showError != "" && (
        <Alert variant="error" title={showError} message={showError} />
      )}
    </div>
  );
}
